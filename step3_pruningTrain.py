...
args.job_dir = 'output/step3_pruningModels/'+args.arch
...
def load_vgg_model(model, oristate_dict):
    ...
    cnt=0
    prefix = args.sorted_conv_prefix+'/conv_'
    subfix = ".npy"
    for name, module in model.named_modules():
        ...
        if isinstance(module, nn.Conv2d):
            ...
            if orifilter_num != currentfilter_num:
                ...
                cov_id = cnt
                logger.info('loading sorted from: ' + prefix + str(cov_id) + subfix)
                sorted = np.load(prefix + str(cov_id) + subfix)
                select_index = sorted[:currentfilter_num] # preserved filter id
                select_index.sort()
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, Inception):
            ...
            for weight_index in honey_filter_index:
                ...
                if orifilter_num != currentfilter_num:
                    logger.info('loading sorted from: ' + prefix + str(cov_id) + branch_name + subfix)
                    sorted = np.load(prefix + str(cov_id)  + branch_name + subfix)
                    select_index = sorted[:currentfilter_num] # preserved filter id
                    select_index.sort()
...
def load_densenet_model(model, oristate_dict):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, nn.Conv2d):
            ...
            if orifilter_num != currentfilter_num:
                print("load --------", str(cov_id))
                logger.info('loading sorted from: ' + prefix + str(cov_id) + subfix)
                sorted = np.load(prefix + str(cov_id) + subfix)
                select_index = list(sorted[:currentfilter_num]) # preserved filter id
                select_index.sort()
    ...
    if args.resume:
        ...
        if args.use_pretrain:
            ...
            if args.arch=='vgg_16_bn' or args.arch=='resnet_56':
                ...
                ckpt = torch.load(args.pretrain_dir+args.arch+'.pt', map_location=device)
            else:
                ckpt = torch.load(args.pretrain_dir+args.arch+'.pt')
